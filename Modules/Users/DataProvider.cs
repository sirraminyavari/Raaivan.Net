...
namespace RaaiVan.Modules.Users
{
    class DataProvider
    {
        ...
        private static void _parse_phone_numbers(ref IDataReader reader, ref List<PhoneNumber> lstPhoneNumbers)
        {
            while (reader.Read())
            {
                try
        ...
        private static void _parse_email_addresses(ref IDataReader reader, ref List<EmailAddress> lstEmailAddress)
        {
            while (reader.Read())
            {
                try
        ...
        public static bool CreateUser(Guid? applicationId, User info, bool passAutoGenerated)
        {
            ...
            try
            {
                ...
                bool succeed = ProviderUtil.succeed(ProviderUtil.execute_reader(spName, applicationId, info.UserID, 
                    ...
                    PublicMethods.sha1(info.Password), passAutoGenerated, string.Empty, DateTime.Now));
            ...
            Guid? invitationId, ref string errorMessage)
        {
            ...
            try
            {
                ...
                if (!result || !PublicMethods.send_email(applicationId, email, confirmationEmailBody))
                {
        ...
        public static bool SetPassResetTicket(Guid? applicationId, 
            Guid userId, Guid ticket, string email, string emailBody, bool sendEmail)
        {
            bool result = false;
            ...
            try
            {
                ...
                if (sendEmail && (!result || !PublicMethods.send_email(applicationId, email, emailBody)))
                {
                    tran.Rollback();  //if !result the transaction is already rolled back
                    result = false;
                }
        ...
        public static void GetEmailOwners(Guid? applicationId, 
            ref List<EmailAddress> retEmailAddresses, List<string> emails)
        {
            ...
            try
            {
                ...
                _parse_email_addresses(ref reader, ref retEmailAddresses);
            }
            catch (Exception ex)
            {
                LogController.save_error_log(applicationId, null, spName, ex, ModuleIdentifier.USR);
