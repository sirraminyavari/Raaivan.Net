(function () {
    if (window.ConfidentialityLevelsManager) return;

    window.ConfidentialityLevelsManager = function (containerDiv, params) {
        ...
        GlobalUtilities.load_files(["API/PrivacyAPI.js", "Confidentiality/ConfidentialityMembersManager.js"], {
            OnLoad: function () { that._initialize(); }
        });
    }

    ConfidentialityLevelsManager.prototype = {
        _initialize: function () {
            ...
            var elems = GlobalUtilities.create_nested_elements([
                {
                    ...
                    Childs: [
                        { Type: "text", TextValue: RVDic.PRVC.ManageConfidentialityLevels },
                        { Type: "help", Style: "margin-" + RV_Float + ":0.5rem;", Params: { Name: "systemsettings_confidentialitylevels" } }
                    ]
                },
                {
                    ...
                    Childs: [
                        {
                            ...
                            Childs: [
                                {
                                    Type: "number", Params: { MaxLength: 4 }, Class: "rv-input", Name: "idInput",
                                    Style: "width:2rem; font-size:0.7rem; margin-" + RV_RevFloat + ":0.5rem;", InnerTitle: RVDic.Code
                                },
                                {
                                    Type: "input", Class: "rv-input",Name: "itemInput", 
                                    Style: "width:calc(100% - 2.5rem); font-size:0.7rem;",
                                    InnerTitle: RVDic.NewN.replace("[n]", RVDic.Classification) + "..."
                                }
                            ]
                        },
                        {
            ...
            GlobalUtilities.set_onenter(itemInput, function () {
                ...
                PrivacyAPI.AddConfidentialityLevel({
                    LevelID: id, Title: Base64.encode(name), ParseResults: true,
                    ResponseHandler: function (result) {
                        if (result.ErrorText) return alert(RVDic.MSG[result.ErrorText] || result.ErrorText);
                        that.add_item(result.Level);
                    }
                });
        ...
        show_items: function () {
            ...
            PrivacyAPI.GetConfidentialityLevels({
                ParseResults: true,
                ResponseHandler: function (result) {
                    that.Interface.ItemsArea.innerHTML = "";
                    jQuery.each(result.Levels || [], function (ind, val) { that.add_item(val); });
                }
            });
        },
        ...
        add_item: function (params) {
            ...
            var itemId = params.ConfidentialityID;
                            ...
                            Childs: [
                                    ...
                                    Childs: [
                                        {
                                            ...
                                            Childs: [{ Type: "text", TextValue: levelId, Name: "idText" }]
                                        },
                                        {
                                            Type: "div", Style: "display:inline-block;", Tooltip: name,
                                ...
                                },
                                {
                                    ...
                                    Childs: [
                                        {
                                            Type: "number", Params: { MaxLength: 4 }, Class: "rv-input", Name: "idInput",
                                            Style: "width:2rem; font-size:0.7rem; margin-" + RV_RevFloat + ":0.5rem;"
                                        },
                                        {
            ...
            var idInput = elems["idInput"];
            var nameInput = elems["nameInput"];
            var idText = elems["idText"];
            ...
            editButton.onclick = function () {
                var set_things = function () {
                    ...
                    idText.data = GlobalUtilities.convert_numbers_to_persian(itm.LevelID);
                    nameText.data = itm.Title;
                    idInput.value = itm.LevelID;
                ...
                if (this.__Editing === true) {
                    var newId = +GlobalUtilities.trim(idInput.value);
                    ...
                    PrivacyAPI.ModifyConfidentialityLevel({
                        ConfidentialityID: itemId, LevelID: newId, Title: Base64.encode(newName), ParseResults: true,
                        ResponseHandler: function (result) {
                            if (result.ErrorText) alert(RVDic.MSG[result.ErrorText] || result.ErrorText);
                            else {
                                that.Objects.Items[itemId].LevelID = levelId = newId;
                                that.Objects.Items[itemId].Title = name = newName;
        ...
        on_item_select: function (itemId) {
            ...
            if (!item.Settings) {
                ...
                new ConfidentialityMembersManager(_div, { ConfidentialityID: itemId, Options: { Editable: true } });
            }
        ...
        remove: function (itemId, itemName) {
            ...
            GlobalUtilities.confirm(msg, function (result) {
                ...
                PrivacyAPI.RemoveConfidentialityLevel({
                    ConfidentialityID: itemId, ParseResults: true,
                    ResponseHandler: function (result) {
                        if (result.ErrorText) return alert(RVDic.MSG[result.ErrorText] || result.ErrorText);
