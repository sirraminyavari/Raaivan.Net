...
(function ($, window, document) {
    ...
    function Plugin(el, options) {
        ...
        elem.on('tooltip:show ' + options.trigger.show, function () {
            ...
            if (!tip) {
                tip = $('<div class="tooltip"><div class="tooltip-inner"/></div>').css({ position: 'absolute',
                    zIndex: GlobalUtilities.zindex.tooltip()
                });
            ...
            var oldOffset = elem.offset();
            
            interval = setInterval(function () {
                var newOffset = elem.offset();

                if (!GlobalUtilities.is_visible(htmlElem) || oldOffset.top != newOffset.top || oldOffset.left != newOffset.left) {
