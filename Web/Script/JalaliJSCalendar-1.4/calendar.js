...
Calendar.prototype.create = function (_par) {
    ...
    //Ramin's
    if (parent === document.body && window.GlobalUtilities) this.element.style.zIndex = GlobalUtilities.zindex.alert();
    //end of Ramin's
};
...
Calendar.prototype.showAtElement = function (el, opts) {
    ...
    if (!this._CloseOperation) {
        this._CloseOperation = true;

        jQuery(document).bind('click', function (event) {
            if (!jQuery(event.target).closest(el).length && !jQuery(event.target).closest(that.element).length) {
                that.hide();
            }
        });
    }

    this.element.setAttribute("style", "Gesi:Chaghochi;");
    return GlobalUtilities.stick(el, this.element);
    /* end of Ramin's */
